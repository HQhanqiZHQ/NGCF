...
class Metric:
    ...
    def eval_rating(self, idx, test_ratings, test_negatives, K, model):
        """
        holdout(df_test item) evaluates whether it is within K
        """
        items = test_negatives[
            idx]  # negative items [neg_item_id, ... ] (1,100)
        ...
        # prediction
        predict_user = np.full(len(items), user_idx, dtype='int32').reshape(-1,
                                                                            1)  # [[user_id], ...], (101, 1)
        ...
        # top K items with the highest scores
        k_ranked = heapq.nlargest(K, item_to_pre_score,
                                  key=item_to_pre_score.get)

        # holdout whether within k
        # {1:within k, 0:not within k}
    ...
    def evaluate_top_k(self, df_neg, df_test, model, K=10):
        """
        TOP-K metric evaluate using the dataset
        """
        hits = []
        ...
        # [[(user_id, item_id=holdout)], neg_item,... ] (1,100)

        # user sampling
        sample_idx_lst = np.random.choice(len(test_ratings),
                                          int(len(test_ratings) * 0.3))
        for user_idx in sample_idx_lst:  # range(len(test_ratings))

            hitrate = self.eval_rating(user_idx, test_ratings, test_negatives,
                                       K, model)
