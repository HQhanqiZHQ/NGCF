...
class Run:
    ...
    def __init__(self):
        ...
        user_input, item_input, labels = loader.get_train_instances(uids, iids,
                                                                    num_neg,
                                                                    len(self.items))
        ...
        # input data 준비
        user_data_shuff, item_data_shuff, label_data_shuff = shuffle(user_input,
                                                                     item_input,
                                                                     labels)
    ...
    def run(self):
        nmf = NeuMF(len(self.users),
                    len(self.items))
        self.model = nmf.get_model()
        self.model.fit([self.user_data_shuff, self.item_data_shuff],
    ...
    def calculate_top_k_metric(self):
        metric = Metric()
        hit_lst = metric.evaluate_top_k(self.df_neg, self.df_test, self.model,
                                        K=10)
        hit = np.mean(hit_lst)
...
if __name__ == '__main__':
    ...
    print('top k metric:', top_k_metric)

    # user prediction
    user_id = 0
    user_candidate_item = np.array([134, 6783, 27888, 8362, 25]).reshape(-1, 1)
    user_input = np.full(len(user_candidate_item), user_id,
                         dtype='int32').reshape(-1, 1)

    predictions = model.predict([user_input, user_candidate_item])
    predictions = predictions.flatten().tolist()
    item_to_pre_score = {item[0]: pre for item, pre in
                         zip(user_candidate_item, predictions)}
    item_to_pre_score = dict(
        sorted(item_to_pre_score.items(), key=lambda x: x[1], reverse=True))
